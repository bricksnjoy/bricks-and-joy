...
export default function Orders() {
  ...
  // Orders arrive newest-first, so the first occurrence of a customer/product
  // in the list is its recency rank. Recently used entries float to the top of
  // the pickers; never-used ones follow alphabetically.
  function byRecentUse(list, key) {
    const rank = new Map()
    orders.forEach((o, i) => { const k = o[key]; if (k && !rank.has(k)) rank.set(k, i) })
  ...
  const customerOptions = byRecentUse(customers, 'customer_id').map(c => ({ value: c.id, label: c.name }))
  const productOptions = byRecentUse(products, 'product_id').map(p => ({ value: p.id, label: p.name, hint: `${p.stock_qty ?? 0} in stock` }))
      ...
      {modal && (
        <Modal title={editOrder ? `Edit order — ${editOrder.invoice_number || ''}` : 'New order'} onClose={() => { setModal(false); setScanning(null) }} width={600} noBackdropClose>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <SearchSelect value={form.customer_id} onChange={handleCustomerChange} options={customerOptions}
              placeholder="— Select a customer —" invalid={!form.customer_id} />
              ...
              return (
                <div key={idx} style={{ border: '1px solid #eee', borderRadius: 10, padding: '12px', marginBottom: 8, background: '#fafafa' }}>
                  <div style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                    <SearchSelect value={item.product_id} onChange={v => handleProductChange(v, idx)} options={productOptions}
                      placeholder="— Select product —" style={{ flex: 1, minWidth: 0 }} />
