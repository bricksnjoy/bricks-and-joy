...
export default function HelpGuide({ onClose }) {
  ...
  return createPortal((
    <div className="help-overlay" style={{
      ...
      {/* Minimal top bar */}
      <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '14px 18px', flexShrink: 0 }}>
        <button onClick={onClose} className="help-x" title="Close" style={{
          background: '#f0ede8', border: 'none', cursor: 'pointer', color: '#666',
      ...
      <div className="help-scroll" style={{ flex: 1, overflowY: 'auto', padding: '0 26px 60px' }}>
        <div className="help-body-inner">
          ...
          {/* ── DETAIL VIEW ── */}
          {active ? (
            <div style={{ animation: 'helpRise 0.3s ease both' }}>
              <button className="help-back" onClick={() => { setOpenId(null); document.querySelector('.help-scroll')?.scrollTo({ top: 0 }) }}>
              ...
              <div style={{ background: '#fff', border: '1px solid #eee', borderRadius: 16, padding: '14px 18px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                {active.steps.map((s, i) => (
                  <div key={i} className="help-step" style={{ display: 'flex', gap: 13, padding: '12px 10px', borderRadius: 10, transition: 'background 0.15s' }}>
                    <div style={{ flexShrink: 0, width: 26, height: 26, borderRadius: '50%', background: active.color, color: '#fff', fontSize: 13, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{i + 1}</div>
            ...
            <>
              ...
              <div className="help-hero">
                ...
                <p style={{ margin: '0 0 28px', fontSize: 15, color: '#999', fontWeight: 400 }}>
                  Step-by-step guides for everything in the app.
                </p>
                {/* Search */}
                <div style={{ position: 'relative', maxWidth: 520, margin: '0 auto' }}>
                  <Search size={17} color="#aaa" style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} />
                  <input
                    autoFocus
                    value={query}
                    onChange={e => setQuery(e.target.value)}
                    placeholder="Search for anything…"
                    style={{ width: '100%', padding: '14px 18px 14px 46px', border: '2px solid #e8e5df', borderRadius: 99, fontSize: 14.5, fontFamily: 'inherit', outline: 'none', background: '#fff', boxSizing: 'border-box', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', transition: 'border-color 0.15s' }}
              ...
              {filtered.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '50px 0', color: '#c4c4c4' }}>
                ...
                <>
                  ...
                  <div className="help-grid">
                    {filtered.map((g, gi) => (
                      <button key={g.id} className="help-tile"
                        onClick={() => { setOpenId(g.id); document.querySelector('.help-scroll')?.scrollTo({ top: 0 }) }}
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 14.5, fontWeight: 700, color: '#0d1b2a', letterSpacing: '-0.2px' }}>{g.title}</div>
